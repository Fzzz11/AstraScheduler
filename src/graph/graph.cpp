...
// ============================================================================
// TaskGraph 的运行期实现：节点调度、依赖解锁、失败传播与报告。
//
// 【执行模型】
//   冻结后的图在 run() 时按依赖拓扑逐步投放：入度清零的节点变成普通任务
//   提交给调度器；节点完成时把后继的"未满足依赖"计数减一，减到零就投放
//   后继。不需要专门的图线程——节点就是普通任务， worker 池直接复用。
//
// 【失败传播】
//   节点抛异常会被捕获存入报告；默认边策略（RequireSuccess）下，前置失败
//   的后继不再执行、标记为取消。所有终态汇入 GraphReport（含每个失败节点
//   的异常指针），供调用方在 run.wait() 后检查。
//
// 【取消】
//   GraphRun::request_cancel() 置取消标志：未开始的节点不再投放，
//   已开始的靠任务自身响应取消。等待者（wait/get）在图进入终态时被唤醒。
// ============================================================================
#include <algorithm>
#include <map>
#include <queue>
