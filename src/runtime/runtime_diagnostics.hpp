...
class ASTRA_NO_EXPORT RuntimeDiagnostics final {
  public:
    ...
    void attach(const std::shared_ptr<TraceCollector>& collector, std::size_t worker_count);

    [[nodiscard]] bool tracing_enabled() const noexcept;

    void emit_wait_event(bool from_current_worker, std::size_t worker_index, TraceEventKind kind,
                         TaskId source, TaskId target, GraphRunId graph_target,
  ...
  private:
    [[nodiscard]] TraceSlot* select_slot(bool from_current_worker,
...
void emit_wait_trace_event(RuntimeDiagnostics* diagnostics, TraceEventKind kind, TaskId source,
                           TaskId target, GraphRunId graph_target, std::uint16_t reason) noexcept;
